  ...
  return (
    <div className="w-full max-w-3xl text-left">
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-display text-2xl font-bold text-white">Live Comparison</h2>
        <span className="text-sm text-emerald-400 font-medium bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20">
      ...
      >
        {data.rates.map((provider) => (
          <motion.div 
            ...
            variants={item}
            className={`relative overflow-hidden rounded-2xl border bg-slate-900 p-5 md:p-6 transition-all hover:border-emerald-500/50 hover:shadow-xl cursor-pointer ${provider.isBestRate ? 'border-emerald-500/60 ring-1 ring-emerald-500/30' : 'border-slate-800'}`}
          ...
          >
            {provider.isBestRate && (
              <div className="absolute top-0 right-0 bg-emerald-500 text-slate-950 text-xs font-black px-3 py-1 rounded-bl-lg shadow-lg uppercase tracking-wider">
                Highest Return
              </div>
            )}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-4 w-48">
                ...
                <div className="w-12 h-12 rounded-full border border-slate-800 bg-slate-950 flex flex-shrink-0 items-center justify-center font-bold text-lg text-emerald-400 shadow-sm overflow-hidden">
                   {provider.provider[0]}
                </div>
                <div>
                  <h3 className="font-semibold text-lg text-white">{provider.provider}</h3>
                  <p className="text-sm text-slate-400">{provider.transferSpeed}</p>
                </div>
              </div>
              ...
              <div className="flex-1 min-w-[200px]">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm font-medium text-slate-500">Provider Rate:</span>
                  <span className="font-semibold text-slate-200">{srcSymbol}1 = {tgtSymbol}{provider.exchangeRate.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                </div>
              ...
              <div className="text-left md:text-right">
                <div className="text-sm text-slate-500 mb-1">Recipient Gets</div>
